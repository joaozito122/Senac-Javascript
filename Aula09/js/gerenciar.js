...
//Quando clicar no botão cadastrar cliente o botão atualizar do model deve desaparecer, vamos aplicar um estilo css de display none
    ...
    .then((rs) => {
      ...
      rs.dados.map((cli) => {
        saida += `<div class="card col-3">
            ...
            <div class="card-body">
              <a href="#" class="btn btn-danger" onclick=apagar(${cli.idcliente})>Apagar</a>
...
function apagar(id) {
  if (confirm(`Você deseja realmente apagar o usuário?`) == 1) {
    fetch(`http://10.26.49.20:3000/apagar/${id}`, {
      ...
      .then((rs) => {
        if (rs.msg == "Apagou") {
          ...
          //fazer um reload na página para atualizar os dados
          window.location.reload();
        } else {
          alert(
            `Não foi possivel apagar o cliente de id: ${id}\n veja o erro abaixo:\n${rs.msg}`
          );
        }
      })
...
//Vamos aplicar a função de atualizar os dados do formulário ao botão atualizar do modal
btnatualizar.onclick = () => {
  if (confirm(`Você deseja realmente atualizar o usuário?`) == 1) {
    fetch(`http://10.26.49.20:3000/atualizar/${idcli}`, {
      ...
      .then((rs) => {
        if (rs.msg == "Atualizado") {
          ...
          alert(
            `Não foi possivel atualizar o cliente ${nome.value} \n veja o que aconteceu:\n${rs.msg}`
          );
        }
      })
